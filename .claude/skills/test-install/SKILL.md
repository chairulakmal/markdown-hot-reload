...
6. **Launches `mhr` against the fixture under `xvfb-run`**, then checks the process is still alive a few seconds later. A webview that fails to construct (see the `WebViewBuilder::build` trap in the repo's `AGENTS.md`) exits immediately instead of hanging, so a dead process here is a real finding, not flakiness.
7. **Screenshots the root window with ImageMagick's `import`**, before and after writing a `# Reload check` heading to the **top** of the watched file on disk. The two screenshots are the actual evidence: read both with the Read tool (they render as images) to confirm the markdown rendered correctly (headings, code fences, syntax highlighting) and that the new heading is the first thing in the second one, proving the `notify` watcher fired end to end, not just that the process didn't crash. The marker goes at the top because nothing scrolls the window during the run: a screenshot only ever shows the top of the document, so a section appended to the end of a long fixture lands below the fold and both screenshots look identical even when reload works.

## After it runs

Read `$OUT_DIR/before.png` and `$OUT_DIR/after.png` yourself and describe what they show; do not report success on process-alive alone. `after.png` must open with the `Reload check` heading. If it looks the same as `before.png`, the watcher did not fire, and that is a real finding to report rather than an artifact of where the marker landed. If anything looks wrong, `$OUT_DIR/mhr.log` has the process's stderr/stdout, and `--keep` leaves the container up for `docker exec -it <name> bash` to dig further.
