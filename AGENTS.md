...
- **A GTK app id is a session-bus name, and strict confinement refuses to let a snap own one it has not declared.** `with_app_id` in `src/main.rs` reaches `gtk::Application::new`, and tao then calls `register`, which asks the session bus for that name. The AppArmor profile snapd generates permits a snap to own nothing but `org.kde.StatusNotifierItem-*`, so the request comes back `AccessDenied` and tao's `.expect("Failed to initialize gtk backend!")` aborts the process before a window opens. The `dbus-mhr` slot in `snap/snapcraft.yaml` grants the name through the interface's permanent slot rules, which apply on install with no connection needed. Three places carry that string and must match: the slot, the `with_app_id` call, and `StartupWMClass` in `packaging/mhr.desktop` and `snap/gui/mhr.desktop`. Only confinement triggers this, so the deb and a `cargo run` never see it, and it is invisible until the snap runs on a real machine. Renaming the id does not avoid it: no name is allowed by default, whatever it is called.
