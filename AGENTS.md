...
One Rust binary. `comrak` renders the markdown, `render::sanitize` filters the result against an `ammonia` allowlist, `notify` watches the file's parent directory, and `wry` and `tao` host a system webview that receives the HTML through `evaluate_script`. The frontend is a static HTML shell plus a small amount of vanilla JavaScript, compiled in by `rust-embed`. [`README.md`](README.md) covers the architecture and supported feature set in full.
...
- **Raw HTML renders as the GitHub-safe subset, filtered by `render::sanitize`.** `render.r#unsafe` is true, so comrak passes raw HTML through and the tags GitHub allows (tables, `<details>`, `<kbd>`, alignment) render. Every string then goes through `sanitize`, an `ammonia` allowlist that keeps only the elements, attributes and URL schemes named in `src/render.rs` and drops `<script>`, `<style>`, event handlers, inline `style`, and non-web URLs. Documents come from agents and editors, so this filter is the safety boundary: `src/render.rs` tests assert both halves (the subset renders, the dangerous parts do not survive). Turning `render.r#unsafe` back to false, or weakening the allowlist, is a design change, not a refactor.
...
- **Parsing, highlighting, escaping, and sanitization happen in Rust.** JavaScript only morphs the DOM and draws diagrams.
...
cargo test --locked                  # render and its escaping and sanitization guarantees, math validation, CLI parsing, assets, the watcher
...
- **`render::sanitize` runs over the whole document, not just the document-authored spans, so its allowlist has to be a superset of everything comrak and `src/math.rs` emit.** A tag the renderer generates that is missing from the `ammonia::Builder` config is stripped from the output with no error. This already bit the footnote wrapper (`<section>`) and the task-list checkbox (`<input>`), neither of which is in ammonia's default tag set. When a render test starts failing on a missing element after a comrak or math change, add the element to `sanitize` before assuming the renderer is wrong. `math::ELEMENTS` and `math::ATTRIBUTES` are `pub(crate)` for exactly this reuse. One attribute is dropped on purpose: `style` is filtered out of `math::ATTRIBUTES` because the page carries no inline styles (see the syntect trap above), and ammonia rewrites every `<a href>` with `rel="noopener noreferrer"`.
- **An allowlist that names an attribute allows every value of it, which is not always what the invariant needs.** Three places in `sanitize` therefore constrain values rather than names. `data:` is in `url_schemes`, because an embedded image is the only image an offline viewer can show, and `is_embedded_image` is what keeps `data:text/html` and `data:image/svg+xml` off the page; both execute script in a webview. `input` gets its `type` pinned to `checkbox` through `add_tag_attribute_values` and `disabled` forced on through `set_tag_attribute_value`, because naming `type` in `add_tag_attributes` would let a document open a working text box inside a read-only window. And the app's own notices are marked `data-mhr-notice`, not a class, because `class` is allowed on everything and a document could otherwise render its own text styled as a system message from `mhr`. Each has a test in `src/render.rs`; a change that loosens one passes the others.
