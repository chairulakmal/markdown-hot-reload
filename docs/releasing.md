...
13. Check out a worktree at the tag, not the branch that produced it: `git worktree add --detach <path> vX.Y.Z`. A branch can carry commits the tag does not, and the next two steps have to be against exactly what the tag names. From `<path>`, run `.claude/skills/test-install/scripts/test-install.sh --cargo-install-path <path>`. This packages the worktree the way `cargo publish` would, applying `Cargo.toml`'s `exclude` list, and confirms the resulting binary actually opens a window under Xvfb before crates.io ever sees it. Do this before the next step, not after: a crates.io version can be yanked but never replaced or deleted, so a build that only fails for someone running `cargo install mhr` has to be caught here.
14. Publish to crates.io from the same worktree: `cargo publish --locked` from `<path>`, then `git worktree remove <path>`. The token needs the `publish-update` scope; the `publish-new` scope was only for creating the crate, at 0.1.0, and is not needed again. The crates.io listing page, README included, is a snapshot taken at publish time; nothing there updates again until the next `cargo publish`.
15. Review the GitHub Release draft, replace the generated notes with real ones, publish.
16. Promote the revision already sitting on `edge`, rather than uploading again: `snapcraft release <snap> <revision> stable`. This way the bits verified in step 10 are exactly the bits that get released.
